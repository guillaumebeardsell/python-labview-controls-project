...
| `APC_9049_CycleAvgSignals.vi` | Name registry for the 9049 cycle-averaged variable array *(inference from use by pollers/mapping)*. |
...
| `APC_9056_WarningBool.vi`, `ErrorMask.vi`, `MaskErrors.vi`, `MergeCylErrors.vi`, `ClearSoftWarning.vi`, `ForceArraySize16.vi`/`6b.vi` | Warning-pipeline helpers: threshold booleans, mask application, per-cylinder error merge, self-clearing logic, array-size normalization *(inference from names/wiring)*. |
| `APC_0_100_to_4_20mA.vi` / `APC_rpm_to_4_20mA.vi` | Scale 0–100 % / rpm → 4–20 mA AO values. |

### APC_9056_FPGA_main.vi — 9056 FPGA (bridge only)
Pure I/O bridge between the C-series modules (NI 9375 DIO, 9264 AO 0–10 V, 9266 AO
4–20 mA) and RT — no control logic [Report §B.2]. Separate 9053/9056 variants existed for
the lab-vs-plant chassis swap.
